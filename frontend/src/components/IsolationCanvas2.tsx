...
console.log('IsolationCanvas loaded - VERSION 2.0 with custom shapes');
...
export function IsolationCanvas({
  ...
  const getPointAtPosition = useCallback((x: number, y: number): IsolationPoint | null => {
    const pointRadius = 28; // Increased to account for larger shapes
  ...
  const drawPoint = useCallback((
    ...
    const baseSize = 22;
    const size = isSelected ? baseSize + 3 : isHovered ? baseSize + 1 : baseSize;
    ...
    // Draw tag number on top of shape
